...
int Kruskal_unbranching_cycled(std::vector <std::vector<int>> M) // аналог алгоритма краскала дл€ находжени€ первой верхней границы
{
    ...
    for (auto elem : V) // пробег по всем ребрам
    {
        ...
        if (sets.size() == 1 && cur == bound && sets[0].size() == M.size()) break; // выход, когда получили достаточное дл€ гамильтонова цикла количество ребер
    }
    //printf(sets[0]);
    res += M[sets[0][sets[0].size() - 1]][sets[0][0]]; //«амыкаем кольцо
...
int Kruskal(std::vector <std::vector<int>> M) // классический алгоритм  раскала дл€ находжени€ минимального остовного дерева
{
